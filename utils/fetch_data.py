...
API_E03_URL = "https://tradereport.moc.go.th/stat/reportcomcodeexport03/result"

def fetch_years(api_url, default, lang="en"):
    try:
        response = requests.get(f"{api_url}?lang={lang}", verify=False)
        response.raise_for_status()  # Raise an error for HTTP issues
        years = response.json()  # Parse the JSON response
        

        # Validate the structure and return the list
        if isinstance(years, list):
            return [{"id": str(year.get("id", "")), "text": str(year.get("name", ""))} for year in years]
        else:
            print("Invalid response structure from the API.")
            return default
            ...
            try:
                ...
                if "records" in data and data["records"]:  # Check if 'records' key exists and is not empty
                    ...
                    for record in filtered_records:
                        ...
                        record["month_id"] = month["id"]
                        record["month_name"] = month["text"]
...
def fetch_export03(payload, selected_countries, format):
    # File paths
    region_file_path = f"data/{format}_e03_region.json"
    countries_file_path = f"data/{format}_e03_countries.json"
    ...
    for country in selected_countries:
        ...
        for comcode in comcodes:
            sleep_time = random.uniform(1, 3)  # Random sleep between 1 and 5 seconds
            time.sleep(sleep_time)
            payload["comcode"] = comcode
            # st.write(payload)
            try:
                # Make the API request
                response = requests.post(API_E03_URL, json=payload, headers=headers, verify=False)
                response.raise_for_status()  # Raise an error for HTTP issues
